...
export const Route = createFileRoute("/jumeau-numerique")({
  head: () => ({
    meta: [
      { title: "Jumeau numérique, Aquapure Analytics" },
      {
        name: "description",
        content:
        ...
        <div className="relative grid lg:grid-cols-[minmax(0,280px)_auto_minmax(0,320px)] gap-y-8 lg:gap-x-16 xl:gap-x-24 items-center justify-items-center">
          <div className="w-full max-w-[280px] lg:justify-self-end">
            <article className="diagram-node diagram-node--violet p-5 text-left">
              ...
              <p className="text-sm font-bold text-navy leading-snug">Base AquaPure auto-hébergée</p>
              <p className="text-xs text-muted-foreground mt-1">Votre propre base de données</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {["PostgreSQL", "MongoDB", "SQL"].map((db) => (
              ...
              <p className="mt-3 text-[11px] font-semibold text-primary">Branché à vos automates</p>

              <div className="mt-5 pt-5 border-t border-border/50">
                <p className="text-sm font-bold text-navy">Sources externes</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {[
                    { label: "JSON", color: "text-amber-700" },
                    { label: "CSV", color: "text-emerald-700" },
                    { label: "Excel", color: "text-green-700" },
                    { label: "Airtable", color: "text-sky-700" },
                  ].map((f) => (
          ...
          <div className="w-full max-w-[320px] lg:justify-self-start">
            <article className="diagram-node diagram-node--sky p-5 md:p-6 text-left space-y-4">
              <p className="text-sm font-bold text-navy">Plateforme de contrôle AquaPure</p>
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {["Grille", "Moniteur", "Éditeur", "Formulaire"].map((t) => (
                    <span key={t} className="diagram-format text-[11px]">
                      {t}
                    </span>
                  ))}
                </div>
                <p className="mt-2 text-[11px] font-semibold text-primary">Et plus encore !</p>
              </div>

              <div>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                  API et serveur locaux
                </p>
                <div className="mt-2 space-y-1.5">
                  {["Web", "Applications tierces", "Mobile"].map((t) => (
              ...
              <div className="rounded-xl border border-border/50 bg-white p-3">
                <p className="text-[10px] font-semibold text-muted-foreground leading-snug">
                  Requête, automatiser le contrôle et visualiser les données
                </p>
                <div className="mt-3 h-[4.5rem] rounded-lg bg-gradient-to-br from-slate-50 to-white border border-border/40 relative overflow-hidden">
                  <svg className="absolute inset-2 w-[calc(100%-1rem)] h-[calc(100%-1rem)]" viewBox="0 0 200 60" aria-hidden>
                    <polyline
                      points="0,45 30,38 55,42 80,22 105,28 130,15 155,25 180,10 200,18"
                      fill="none"
...
const WHY_FEATURES: { icon: LucideIcon; title: string; desc: string }[] = [
  {
    icon: LineChart,
        ...
        <div className="mx-auto max-w-5xl text-center">
          <Reveal>
            ...
            <p className="mt-5 text-base md:text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              Découvrez comment AquaPure connecte vos bases de données et exploite un agent IA auto-hébergé
              pour automatiser vos procédés industriels et optimiser le contrôle en temps réel.
            </p>
          </Reveal>
        ...
        <div className="mx-auto max-w-7xl">
          <Reveal>
            <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight">Pourquoi AquaPure ?</h2>
              <p className="mt-4 text-sm md:text-base text-white/60 leading-relaxed">
                Automatisez le contrôle industriel, réduisez la consommation d&apos;énergie et prenez des décisions
                fondées sur des données fiables, sans compromettre la sécurité ni la souveraineté de vos
                informations.
              </p>
            </div>
          </Reveal>
            ...
            {WHY_FEATURES.map((f, i) => (
              <Reveal key={f.title} delay={i * 60} className="h-full">
                <article className="feature-card-nocodb group h-full">
                  <f.icon
                    className="size-8 text-white/85 mb-6 transition-transform duration-300 group-hover:scale-105"
                    strokeWidth={1.25}
                  />
                  <h3 className="font-bold text-base md:text-[1.05rem] text-white leading-snug pr-2">
                    {f.title}
                  </h3>
        ...
        <div className="mx-auto max-w-7xl text-center">
          <Reveal>
            ...
            <p className="mt-3 text-sm md:text-base text-muted-foreground max-w-2xl mx-auto leading-relaxed">
              Une technologie de pointe propulsée par des acteurs de confiance pour accélérer la transition
              numérique de votre industrie.
            </p>
          </Reveal>
          <Reveal delay={100}>
        ...
        <div className="mx-auto max-w-7xl">
          <Reveal>
            <h2 className="text-center text-2xl md:text-3xl font-bold text-black">
              Quel est votre domaine ? Contactez-nous et parlons de votre projet !
            </h2>
          </Reveal>
          <div className="mt-10 grid md:grid-cols-2 gap-6">
            {[
              { img: indWater, label: "Pour les municipalités", href: "/#contact" },
              { img: indMine, label: "Pour l'industrie", href: "/#contact" },
            ].map((card, i) => (
              <Reveal key={card.label} delay={i * 100}>
                <a
                  href={card.href}
                  className="group relative block aspect-[16/9] overflow-hidden rounded-2xl border border-border/60 hover-lift"
                >
                  <img
                    src={card.img}
                    alt={card.label}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <span className="rounded-md bg-white text-navy px-5 py-2.5 text-sm font-semibold shadow-md group-hover:bg-white/90 transition-colors">
                      Contactez-nous
                    </span>
                  </div>
                  <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/60 to-transparent pointer-events-none" />
                  <span className="absolute bottom-6 left-6 text-xl md:text-2xl font-bold text-white">
                    {card.label}
                  </span>
                </a>
              </Reveal>
